...
function handleAuthRedirects(
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(`[Auth Middleware] Path: ${pathname}, Logged in: ${isLoggedIn}`)
  }
  ...
  const authPages = [
    `/${countryCode}/auth/login`,
    `/${countryCode}/auth/register`,
    `/${countryCode}/auth/forgot-password`,
    `/${countryCode}/auth/reset-password`,
    `/${countryCode}/auth/signin`,
    `/${countryCode}/auth/signup`,
  ]
  ...
  // If user is logged in and trying to access auth pages, redirect to my-account
  if (isLoggedIn && authPages.some((page) => pathname.startsWith(page))) {
    return NextResponse.redirect(
      new URL(`/${countryCode}/account/my-account`, request.url),
    )
  ...
  if (
    ...
    return NextResponse.redirect(
      new URL(`/${countryCode}/auth/login`, request.url),
    )
...
export async function middleware(request: NextRequest) {
  ...
  // Handle authentication redirects if we have a country code
  if (countryCode) {
    const authRedirect = handleAuthRedirects(request, countryCode)
    if (authRedirect) {
      return authRedirect
    }
  }

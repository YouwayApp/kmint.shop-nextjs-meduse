...
const SingleListItem = ({ item }: SingleListItemProps) => {
        ...
        <div className="shadow-list relative overflow-hidden flex items-center justify-center max-w-[270px] w-full sm:min-h-[270px] p-4">
          ...
          <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
            <button
              ...
              aria-label="hızlı görüntüleme butonu"
            ...
            <button
              ...
              aria-label="favori seçme butonu"
              className="flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 text-dark bg-white hover:text-blue"
            >
              <svg
                className="fill-current"
            ...
            <span className="flex items-center gap-2 font-medium text-lg">
              {price ? (
                <span className="text-dark">
                  {convertToLocale({
                    amount: price.calculated_amount / 100,
                    currency_code: price.currency_code || "USD",
                  })}
                </span>
              ) : (
                <span className="text-gray-400">Fiyat mevcut değil</span>
              )}
            </span>
          </div>

...
import { useCart, useCartTotals } from "@/providers/CartProvider";

const CardBtn = () => {
  const { total, itemCount, error } = useCartTotals();
    ...
    <button onClick={handleOpenCartModal} className="flex items-center gap-2.5">
      <span className="inline-block relative">
        ...
        <span
          ...
          className="flex items-center justify-center font-medium text-2xs absolute -right-2 -top-2.5 bg-blue w-4.5 h-4.5 text-white"
        >
          {loading ? "..." : itemCount}
        </span>
      </span>
      ...
      <div>
        <span className="block text-2xs text-dark-4 uppercase text-left">
          Sepetim{" "}
        </span>
        <p className="font-medium text-custom-sm text-dark">
          {loading
            ...
            : cart
            ? convertToLocale({
                amount: cart.item_total, // Convert from cents to dollars
